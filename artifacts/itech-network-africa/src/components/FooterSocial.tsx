...
export const FooterSocial: React.FC = () => {
  const socials = [
    { icon: <Facebook size={28} />, label: 'Facebook', href: '#' },
    { icon: <Instagram size={28} />, label: 'Instagram', href: '#' },
    { icon: <TikTokIcon />, label: 'TikTok', href: '#' },
    { icon: <XIcon />, label: 'X (Twitter)', href: '#' },
    { icon: <Youtube size={28} />, label: 'YouTube', href: '#' },
  ];

  return (
    <div className="flex flex-row items-center gap-6 py-6 flex-wrap-reverse sm:flex-nowrap">
      {socials.map((social) => (
        <a
          key={social.label}
          href={social.href}
